...
```
python [-u] app.py <input>
```

здесь `<input>`  - обязательное имя текстового файла, подлежащего озвучке. Файл должен содержать текст на русском языке в кодировке UTF-8.
...
В озвучиваемом файле можно использовать разметку SSML - Speech Synthesis Markup Language. Отдельно заметим, что использовать тег `<speak>` в тексте нельзя, т. к. он добавляется скриптом автоматически.
...
* `cardinal` - rоличественное числительное
